...
Trident 可以透過以下兩種方式之一建立 Cloud Volumes Service 磁碟區link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["服務類型"^]：

* *CVS-Performance*：Trident 的預設服務類型。這種效能最佳化型服務類型最適合注重效能的生產工作負載。CVS-Performance 服務類型是一種硬體選項，支援最小 100 GiB 的磁碟區。您可以選擇以下選項之一link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["三個服務級別"^]：
+
** `standard`
** `premium`
** `extreme`
...
* *CVS*：CVS 服務類型提供高區域可用性，但效能水準有限或中等。CVS 服務類型是一種軟體選項，使用儲存池來支援小至 1 GiB 的磁碟區。儲存池最多可包含 50 個磁碟區，所有磁碟區共用集區的容量和效能。您可以選擇以下其中一項link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["兩種服務級別"^]：
+
** `standardsw`
...
.你需要什麼
要設定和使用 https://www.netapp.com/google-cloud/netapp-volumes/["適用於 Google Cloud 的Cloud Volumes Service"^]後端，您需要以下元件：
