...
選擇適合您的安裝方式。在做出決定之前，您還應該考慮 link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["方法之間的轉換"]。



=== 使用Trident運算符

無論手動部署或使用 Helm，Trident operator 都是簡化安裝和動態管理 Trident 資源的絕佳方式。您甚至可以link:../trident-install/kubernetes-customize-deploy.html["自訂您的Trident操作員部署"]使用 `TridentOrchestrator`自訂資源（CR）中的屬性。
