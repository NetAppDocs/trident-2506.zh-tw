...
.開始之前
登入 Linux 主機，並驗證它是否正在管理一個正常運作的 link:../trident-get-started/requirements.html["支援的 Kubernetes 集群"]，以及您是否擁有必要的權限。
...
現在您可以建立  `TridentOrchestrator` 並安裝 Trident。您也可以選擇進一步link:../trident-install/kubernetes-customize-deploy.html["自訂您的Trident安裝"]，方法是使用 `TridentOrchestrator`規格中的屬性。以下範例顯示 Trident 和 CSI 映像檔位於不同註冊中心的安裝情況。
