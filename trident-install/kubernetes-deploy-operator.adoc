...
summary: 您可以手動部署Trident操作員來安裝Trident。此程序適用於Trident所需的容器鏡像未儲存在私人鏡像倉庫中的安裝情況。 
---
= 手動部署Trident操作員（標準模式）
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
您可以手動部署Trident操作員來安裝Trident。此程序適用於Trident所需的容器鏡像未儲存在私人鏡像倉庫中的安裝情況。如果您確實擁有私有鏡像倉庫，請使用link:kubernetes-deploy-operator-mirror.html["離線部署流程"]。
...
.開始之前
在開始安裝之前，請登入 Linux 主機並驗證它是否正在管理運作中的 link:../trident-get-started/requirements.html["支援的 Kubernetes 集群"]，以及您是否擁有必要的權限。
...
現在您可以建立  `TridentOrchestrator` 並安裝 Trident。或者，您可以link:../trident-install/kubernetes-customize-deploy.html["自訂您的Trident安裝"]使用  `TridentOrchestrator` 規範中的屬性。
