...
--
====
. （可選）在來源叢集上，對來源應用程式進行快照。此快照將用作目標叢集上應用程式的基礎。如果跳過此步驟，則需要等待下一次排程快照運行，以便取得最新的快照。
+
[NOTE]
====
...
*** *spec.storageClassName*: (_必填_) 選擇叢集上有效的儲存類別的名稱。儲存類別必須連結到與來源環境建立對等連線的ONTAP儲存 VM。
*** *spec.recurrenceRule*: 定義 UTC 時間的開始日期和重複間隔。
+
YAML 範例：
...
--
.. 建立並套用 AppMirrorRelationship 對象，將括號中的值替換為您環境中的資訊。例如：
+
[source, console]
----
