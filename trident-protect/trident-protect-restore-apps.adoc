...
* 您可以使用進階設定來自訂恢復操作。欲了解更多信息，請參閱 link:../trident-protect/trident-protect-restore-settings.html["使用進階Trident保護恢復設定"]。
...
NOTE: 將備份還原到具有現有資源的不同命名空間不會變更與備份中資源同名的任何資源。若要還原備份中的所有資源，請刪除並重新建立目標命名空間，或將備份還原到新的命名空間。
...
NOTE: 當您使用 Kopia 作為資料移動器還原備份時，您可以選擇在 CR 中指定註解或使用 CLI 來控制 Kopia 使用的暫存的行為。請參閱 https://kopia.io/docs/getting-started/["科皮亞文檔"^]有關您可以配置的選項的詳細資訊。使用 `tridentctl-protect create --help`有關使用Trident Protect CLI 指定註釋的更多信息，請參閱命令。
...
--
.步驟
. 將備份還原到不同的命名空間，並將括號中的值替換為您環境中的資訊。這 `namespace-mapping`此參數使用冒號分隔的命名空間，將來源命名空間對應到正確的目標命名空間，格式如下： `source1:dest1,source2:dest2` 。例如：
+
[source, console]
----
...
--
.步驟
. 將備份還原到原始命名空間，並將括號中的值替換為您環境中的資訊。這 `backup`參數使用命名空間和備份名稱，格式如下 `<namespace>/<name>`。例如：
+
[source, console]
----
...
+
** 請參閱 https://docs.aws.amazon.com/STS/latest/APIReference/API_GetSessionToken.html["AWS API 文件"^]有關檢查當前會話令牌過期時間的詳細資訊。
** 請參閱 https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_use-resources.html["AWS 文件"^]有關 AWS 資源憑證的詳細資訊。
...
NOTE: SnapshotRestore 支持 `spec.storageClassMapping`屬性，但僅當來源儲存類別和目標儲存類別使用相同的儲存後端時才有效。如果您嘗試恢復到 `StorageClass`如果使用不同的儲存後端，則復原操作將會失敗。
