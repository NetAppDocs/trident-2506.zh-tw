...
安裝和升級 Trident 的眾多link:../trident-install/kubernetes-deploy.html["使用Trident運算子的好處"]之一是能夠自動處理 Trident 和 Kubernetes 物件，而不會中斷現有掛載的磁碟區。這樣，Trident 可以支援零停機升級，或link:https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/["捲動更新"^]。具體來說，Trident Operator 與 Kubernetes 叢集通訊以：
